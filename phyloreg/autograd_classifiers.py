...
class MiniBatchOptimizer(object):
    ...
    """
    def __init__(self, learning_rate, patience, tolerance, batch_size, max_epochs):
    ...
    def minimize(self, learner, X, y, X_orthologs, species_graph_adjacency):
        ...
        # Gradient decent
        try:
            sgd(batch_gradient, w, callback=log_progress, num_iters=self.max_epochs * epoch_steps, step_size=self.learning_rate, mass=0.9)
...
class AutogradRidgeRegression(BaseEstimator, ClassifierMixin, BasePhyloLearner):
    ...
    def _objective(self, w, X, y, X_orthologs, species_graph_adjacency):
        return self._objective_squared_loss(w, X, y) / X.shape[0] + \
               self.alpha * self._objective_l2_norm(w) + \
               self.beta * self._objective_orthologs(w, X_orthologs, species_graph_adjacency) / X.shape[0]
...
class AutogradLogisticRegression(BaseEstimator, ClassifierMixin, BasePhyloLearner):
    ...
    def _objective(self, w, X, y, X_orthologs, species_graph_adjacency):
        return self._objective_negative_log_likelihood(w, X, y) / X.shape[0] + \
               self.alpha * self._objective_l2_norm(w) + \
               self.beta * self._objective_orthologs(w, X_orthologs, species_graph_adjacency) / X.shape[0]
